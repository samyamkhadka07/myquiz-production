...
describe('functional separation and engagement contracts',()=>{
 it('gives purpose-specific Admin navigation unique routes',()=>{const source=fs.readFileSync('src/components/admin-shell.tsx','utf8');for(const route of ['/admin/questions/verification','/admin/questions/publication','/admin/blueprints','/admin/reading-materials','/admin/duplicates','/admin/reports','/admin/ingestion','/admin/ai-usage'])expect(source).toContain(route);expect(source).toMatch(/label:'Reading Materials',href:'\/admin\/reading-materials'/);expect(source).toMatch(/label:'Reports',href:'\/admin\/reports'/);expect(source).toMatch(/label:'AI Review \/ Usage',href:'\/admin\/ai-usage'/);});
 it('keeps Student preparation fields out of Admin Profile',()=>{const page=fs.readFileSync('src/components/admin-profile-form.tsx','utf8');expect(page).toContain('SUPER ADMIN · PROTECTED');expect(page).not.toContain('Target score');expect(page).not.toContain('Exam program');});
 it('offers persisted, distinct learning modes rather than decorative links',()=>{const source=fs.readFileSync('src/components/interactive-games.tsx','utf8');for(const mode of ['RAPID_FIRE','RAPID_RECALL','MISTAKE_RESCUE','ACCURACY','DAILY_CHALLENGE'])expect(source).toContain(mode);expect(source).toContain('learning-games');expect(source).toContain('does not change an official MEC test score');});
});
