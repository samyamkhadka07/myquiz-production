...
-- Repair a production lineage in which migration 0007 was recorded without its
-- identity RPCs and execute revocations. Everything here is idempotent so a
-- correctly migrated database receives the same final privileges.
...
drop policy if exists published_question_read on public.questions;
drop policy if exists question_staff_write on public.questions;
drop policy if exists question_staff_read on public.questions;
...
create or replace function public.require_staff() returns uuid language plpgsql stable security definer set search_path=public,pg_temp as $$
...
begin
 insert into profiles(id,display_name) values(new.id,left(coalesce(nullif(btrim(new.raw_user_meta_data->>'display_name'),''),nullif(split_part(new.email,'@',1),''),'Student'),80)) on conflict(id) do nothing;
 insert into entitlements(user_id) values(new.id) on conflict(user_id) do nothing; return new;
end $$;
create or replace function public.update_profile(p_name text,p_target numeric,p_timezone text,p_program uuid) returns public.profiles language plpgsql security definer set search_path=public,pg_temp as $$
...
create or replace function public.has_premium() returns boolean language sql stable security definer set search_path=public,pg_temp as $$
...
do $$ begin
 if not exists(select 1 from pg_constraint where conrelid='public.questions'::regclass and conname='verification_values') then alter table public.questions add constraint verification_values check(verification_status in ('UNVERIFIED','VERIFIED','REJECTED')); end if;
 if not exists(select 1 from pg_constraint where conrelid='public.questions'::regclass and conname='publication_values') then alter table public.questions add constraint publication_values check(publication_status in ('DRAFT','PUBLISHED','ARCHIVED')); end if;
 if not exists(select 1 from pg_constraint where conrelid='public.questions'::regclass and conname='published_consistency') then alter table public.questions add constraint published_consistency check(publication_status<>'PUBLISHED' or (lifecycle='PUBLISHED' and verification_status='VERIFIED' and correct_answer is not null and explanation is not null and difficulty is not null and cognitive_level is not null)); end if;
end $$;

create or replace function public.validate_question() returns trigger language plpgsql set search_path=public,pg_temp as $$
begin
...
drop trigger if exists question_validate on public.questions;
...
create or replace function public.save_question(p_id uuid,p_data jsonb) returns uuid language plpgsql security definer set search_path=public,pg_temp as $$
...
begin
 ...
 if p_id is not null then select * into old from questions where id=p_id for update; if not found then raise exception 'Question not found' using errcode='P0002'; end if; if old.lifecycle='PUBLISHED' then raise exception 'Archive the published question before editing' using errcode='22023'; end if; end if;
 ...
 insert into audit_events(actor_id,action,target_type,target_id) values(auth.uid(),'QUESTION_SAVED','question',qid::text); return qid;
end $$;
create or replace function public.transition_question(p_id uuid,p_action text) returns void language plpgsql security definer set search_path=public,pg_temp as $$
declare q questions;
begin
 perform require_staff(); select * into q from questions where id=p_id for update; if not found then raise exception 'Question not found' using errcode='P0002'; end if;
 case p_action
 when 'REVIEW' then if q.lifecycle not in ('STAGED','VALIDATION_REQUIRED','EXTRACTED') then raise exception 'Question is not staged'; end if; update questions set lifecycle='PENDING_REVIEW' where id=p_id;
 when 'VERIFY' then if q.lifecycle<>'PENDING_REVIEW' or q.correct_answer is null or nullif(btrim(q.explanation),'') is null or q.difficulty is null or q.cognitive_level is null then raise exception 'Review, answer, explanation, difficulty and cognitive level are required'; end if; if not (q.option_explanations ?& array['A','B','C','D']) or exists(select 1 from jsonb_each_text(q.option_explanations) e where length(btrim(e.value))=0) then raise exception 'An explanation for every option is required'; end if; update questions set lifecycle='VERIFIED',verification_status='VERIFIED',verified_by=auth.uid(),verified_at=now() where id=p_id;
 when 'PUBLISH' then if q.lifecycle<>'VERIFIED' or q.verification_status<>'VERIFIED' then raise exception 'Only verified questions can be published'; end if; update questions set lifecycle='PUBLISHED',publication_status='PUBLISHED',published_at=now() where id=p_id;
...
grant execute on function public.add_flashcard(uuid),public.write_comment(uuid,uuid,uuid,text),public.react_comment(uuid,text),public.report_comment(uuid,text),public.moderate_comment(uuid,text) to authenticated;
...
grant execute on function public.resolve_report(uuid,text),public.review_contribution(uuid,text),public.save_external_source(uuid,jsonb),public.queue_ingestion(uuid) to authenticated;
