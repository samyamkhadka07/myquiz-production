...
Staff endpoints expose staged editing/import/rejection/revision, processing retry, report resolution and contribution review. Canonical CSV export, source mutation and administration analytics require ADMIN or SUPER_ADMIN. User listing, user-role changes and `admin-requests/:id` approval/rejection require SUPER_ADMIN; ordinary ADMIN and MODERATOR identities cannot enumerate role-management data or grant Admin access. Cron endpoints require a server-only bearer secret and execute bounded checkpointed work.

Authentication redirects are role-aware only after the server loads `profiles.role`: STUDENT → `/dashboard`; approved ADMIN/SUPER_ADMIN (and existing MODERATOR) → `/admin`; pending/rejected requests → `/dashboard` with a status code used to render the corresponding message. Client-submitted account type never becomes an authoritative role.
