...
Staff endpoints expose staged editing/import/rejection/revision, processing retry, canonical CSV export, report resolution, contribution review, authorized sources and manual ingestion. User-role changes and `admin-requests/:id` approval/rejection require SUPER_ADMIN; ordinary ADMIN and MODERATOR identities cannot grant Admin access. Cron endpoints require a server-only bearer secret and execute bounded checkpointed work.
