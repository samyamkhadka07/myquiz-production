...
export async function GET(request:Request){
 ...
 let target=url.searchParams.get('next')==='/reset-password'?'/reset-password':'/dashboard';
 if(result&&!result.error&&target!=='/reset-password'){
  const {data:{user}}=await db.auth.getUser();
  if(user){const [profile,requestRow]=await Promise.all([db.from('profiles').select('role').eq('id',user.id).single(),db.from('admin_role_requests').select('status').eq('user_id',user.id).maybeSingle()]);if(profile.data)target=roleHome(profile.data.role as Role,(requestRow.data?.status??null) as AdminRequestStatus);}
 }
